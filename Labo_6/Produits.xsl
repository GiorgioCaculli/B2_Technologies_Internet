...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="algo" select="'Réf_produit'"/>
    <xsl:template match="dataroot">
        <html>
            ...
            <body>
                <h2>Quantité : <xsl:value-of select="$qte"/> - Algorithme de tri : <xsl:value-of select="$algo"/></h2>
                <table>
                    <tr>
                        <th>Référence</th>
                        <th>Nom</th>
                    ...
                    <xsl:for-each select="Produits[Unités_en_stock > $qte]">
                        <xsl:sort select="*[name() = $algo]" data-type="number"/>
                        <tr>

	...
	<html>
		...
		<body>
			...
			<xsl:for-each select="nuages/nuage">
				<h2><xsl:number format="1" level="single"/>. <xsl:variable name="name" select="nom/text()"/><xsl:value-of select="concat(translate(substring($name,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),substring-after($name,substring($name,1,1)))"/></h2>

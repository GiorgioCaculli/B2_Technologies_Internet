	...
	<html lang="fr">
		...
		<xsl:for-each select="systeme_solaire/planete">
			...
			<xsl:if test="not(nom='Terre')">
				...
				<xsl:choose>
					<xsl:when test="@type='tellurique'"><p>Planète de type tellurique</p></xsl:when>
					<xsl:when test="@type='gazeuse'"><p>Planète pleine de gaz!</p></xsl:when>
					<xsl:otherwise><p>Autre type de planète...</p></xsl:otherwise>
				</xsl:choose>
			</xsl:if>
		</xsl:for-each>
		</body>
